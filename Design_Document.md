...
## 2. Live Demo
**Application URL:** [INSERT YOUR STREAMLIT LINK HERE]
...
3. **Embedding:** Chunks are sent to the Hugging Face Serverless Inference API (`all-MiniLM-L6-v2`), which returns 384-dimensional vector embeddings.
...
- **Decoupled Repositories:** Separating the frontend and backend dependency requirements (`requirements.txt` vs `backend-requirements.txt`) ensures minimal container sizes and prevents dependency conflicts between Streamlit and FastAPI.
